...
namespace Spinx.Web.Modules.Core.Helpers
{
    public static class Enumeration
    {
        public static List<IdNameIntDto> GetAll<TEnum>(LetterCasing letterCasing = LetterCasing.Title) where TEnum : struct
        {
            ...
            myList.ForEach(item =>
            {
                list.Add(new IdNameIntDto { Id = item.Key, Name = item.Value.Humanize(letterCasing) });
            });
        ...
        public static List<IdNameIntDto> GetValues<T>()
        {
            var values = new List<IdNameIntDto>();
            foreach (var itemType in Enum.GetValues(typeof(T)))
            {
                //For each value of this enumeration, add a new EnumValue instance
                values.Add(new IdNameIntDto()
                {
